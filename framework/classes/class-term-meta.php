<?php
    ...
    class Term_Meta{
		 ...
		 * @return 
		 **/
		 function save_term_options( $term_id,  $taxonomy) {
			...
			/*
				$term = get_term($term_id, 'skin-generator-css');
							if($term->parent!='0') :					
								$term_position = get_term_meta($term->parent,'position', true);					
								$_POST['tax']['position'] = 	$term_position +	$_POST['tax']['position'];
							endif;*/
				...
				foreach ($_data as $key => $value) :
					update_term_meta( $term_id, $this->slug.$key, $value);
     	...
     	function render_fields($fileds){
				...
				* @var string $tax taxonomy name fomr GET
    			...
    			foreach ($fileds as $key => $filed) :
    					...
    					$type  = $filed['type'];
    					$name  = $filed['name'];
    					$value = !empty( $term_id ) ? get_term_meta( $term_id, $this->slug.$filed['name'], true ) : $filed['default'];
    					$title = $filed['title'];
    					$desc  = !empty( $filed['desc'] ) ? $filed['desc'] : '';
    					$attr  = !empty( $filed['attr'] ) ? $filed['attr'] : array();		
    
    					if ( $type == 'select' ) :
    						$html.= self::{$type}("tax[$name]", $value  , $filed['choices'], $filed['title'] , $desc, $attr  );
    					else:
		 ...
		 * @return html
		 **/
		static function checkbox( $name = '', $value = '', $label='Title' , $desc ='', $attr= array() ){ 
	
			//change html tags ..of action edit or add
   			$action = !empty($_GET['tag_ID']) ? $_GET['tag_ID'] : '';
   			$tag    = ($action=="edit") ? 'tr' : 'tr';
   			$tag1   = ($action=="edit") ? 'td' : 'td';
   			$tag2   = ($action=="edit") ? 'th' : 'th';  ?>
			...
			<<?php echo $tag ?>  class="form-field term-<?php echo esc_attr ( $name ) ?>-wrap">
    			...
    			<<?php echo $tag2 ?> scope="row"><label for="<?php echo esc_attr( $name ) ?>"><?php echo (	$action ) ? esc_attr( $label ) : ''; ?></label></<?php echo $tag2 ?>>
						
    			<<?php echo $tag1 ?>><label for="<?php echo $name  ?>"><input id="<?php echo $name ?>" type="checkbox" name="<?php echo $name ?>" value="yes" <?php checked(	$value , 'yes'); ?>  ><?php echo !empty( $label ) ? "$label ": ''; ?></label>
    	        	<?php echo self::desc($desc) ?>
    			</<?php echo $tag1 ?>>
    		</<?php echo $tag ?> >
	    ...
	    static function get_post_meta( $post_id ){
			...
			return get_term_meta($post_id, '', TRUE);
